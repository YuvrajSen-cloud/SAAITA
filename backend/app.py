...
def get_gemma_model():
    """Load Gemma model from transformers library."""
    ...
    try:
        ...
        print("[SAAITA] Loading Gemma model from transformers...")
        
        # Try to use the model from local conda environment or HuggingFace hub
        model_name = "google/gemma-2b-it"  # or your specific model name
        
        # Set device
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"[SAAITA] Using device: {device}")
        ...
        print("[SAAITA] Gemma model loaded successfully!")
        return GEMMA_MODEL, GEMMA_TOKENIZER
        
    except Exception as e:
        print(f"[SAAITA] Error loading Gemma model: {e}")
        return None, None


class GemmaChatSession:
    """Chat session using Gemma model from transformers."""
    def __init__(self, history, system_instruction):
        self.history = history
        self.system_instruction = system_instruction
        self.model, self.tokenizer = get_gemma_model()
    ...
    def send_message(self, prompt):
        if not self.model or not self.tokenizer:
            class ErrorResponse:
                def __init__(self, text):
                    self.text = text
            return ErrorResponse("Error: Gemma model failed to load.")
        ...
        try:
            ...
            reply = full_response[len(conversation):].strip()
            ...
            class ErrorResponse:
                def __init__(self, text):
                    self.text = text
            return ErrorResponse(f"(Gemma Model Error) {str(e)}")
...
class OllamaChatSession:
    """Chat session using Ollama API (e.g., Gemma 4)."""
    def __init__(self, history, system_instruction):
        self.history = history
        self.system_instruction = system_instruction
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                class ErrorResponse:
                    def __init__(self, text):
                        self.text = text
                return ErrorResponse(f"Error: Ollama API returned {response.status_code}")
                
        except requests.exceptions.ConnectionError:
            class ErrorResponse:
                def __init__(self, text):
                    self.text = text
            return ErrorResponse(f"Error: Cannot connect to Ollama at {self.ollama_url}. Make sure Ollama is running.")
        except Exception as e:
            class ErrorResponse:
                def __init__(self, text):
                    self.text = text
            return ErrorResponse(f"(Ollama Error) {str(e)}")
...
def get_or_create_ai_session(session_id: str, user_id: int, onboarding_data: str = ""):
    ...
    if provider == "ollama":
        ...
        chat = OllamaChatSession(formatted_history, system_instruction)
    elif provider == "gemma":
        system_instruction = get_system_instruction(onboarding_data or "")
        chat = GemmaChatSession(formatted_history, system_instruction)
    elif provider == "local_native":
        system_instruction = get_system_instruction(onboarding_data or "")
...
def chat_message():
    ...
    if provider == "gemma":
        ...
        try:
            ...
            return jsonify({"error": "Gemma provider requires transformers and torch. Install with: pip install transformers torch"}), 503
    ...
    if ai_sess is None:
        return jsonify({"error": "Failed to initialize AI. Check your configuration."}), 503
