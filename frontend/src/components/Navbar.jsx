...
import { apiGetSystemPrompt, apiUpdateSystemPrompt, apiGetApiKeys, apiUpdateApiKeys, apiGetProvider, apiUpdateProvider } from '../services/api';
...
const Navbar = ({ activeView, setActiveView, user, onLogout }) => {
    ...
    const [providerInfo, setProviderInfo] = useState({ provider: 'gemini' });
    ...
    useEffect(() => {
        if (showProviderModal) {
            apiGetProvider().then(data => setProviderInfo(data)).catch(console.error);
        }
    }, [showProviderModal]);
    ...
    const handleSaveProvider = async () => {
        ...
        try {
            await apiUpdateProvider(providerInfo);
    ...
    const providerModal = showProviderModal ? createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="bg-black/40 backdrop-blur-sm p-4">
            <div className="glass-panel w-full max-w-md bg-white/90 rounded-3xl shadow-2xl p-7 flex flex-col gap-4 max-h-[90vh]">
                <div className="flex items-center justify-between border-b border-black/5 pb-2">
                    <h2 className="text-xl font-headline font-black text-on-surface flex items-center gap-2">
                    ...
                    <button onClick={() => setShowProviderModal(false)} className="text-on-surface-variant hover:text-primary transition-colors bg-black/5 hover:bg-black/10 rounded-full p-1.5 flex items-center justify-center">
                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
                    </button>
                </div>
                
                <p className="text-[13px] font-body text-on-surface-variant mb-2">
                    Choose the AI Provider you want to use. You can use a native auto-downloading local model as an alternative to Gemini.
                </p>
                
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[11px] font-label font-bold text-on-surface-variant tracking-widest uppercase">Provider</label>
                        <select 
                            className="bg-white/70 border border-black/10 rounded-xl px-3 py-2.5 text-sm font-semibold outline-none focus:ring-2 focus:ring-primary/50"
                            value={providerInfo.provider}
                            onChange={e => setProviderInfo({...providerInfo, provider: e.target.value})}
                        >
                            <option value="gemini">Gemini API</option>
                            <option value="ollama">Ollama (Local Gemma 4)</option>
                            <option value="local_native">Local Native (~1.6GB Gemma)</option>
                        </select>
                    </div>
                </div>

                <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-black/5">
                    <button onClick={() => setShowProviderModal(false)} className="px-5 py-2.5 font-bold text-sm text-on-surface-variant hover:bg-black/5 rounded-full transition-colors active:scale-95">Cancel</button>
                    <button onClick={handleSaveProvider} disabled={isSavingProvider} className="px-6 py-2.5 font-bold text-sm text-white bg-gradient-to-r from-primary to-secondary hover:shadow-lg shadow-md rounded-full transition-all disabled:opacity-50 active:scale-95 flex items-center gap-2">
                        {isSavingProvider ? "Saving..." : "Save Settings"}
                    </button>
                </div>
            </div>
        </div>, document.body
