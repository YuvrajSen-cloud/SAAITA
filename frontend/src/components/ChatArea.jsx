...
const ChatArea = () => {
    const { isVoiceActive, speak } = useVoice();
    ...
    useEffect(() => {
        const handleVoiceChatCommand = (e) => {
            const text = e.detail?.text || '';
            if (text.trim()) {
                setInputValue(text);
                speak(`Got it, sending: ${text}`);
            }
        };

        window.addEventListener('voiceChatCommand', handleVoiceChatCommand);
        return () => window.removeEventListener('voiceChatCommand', handleVoiceChatCommand);
    }, [speak]);
    ...
    useEffect(() => {
        if (isVoiceActive && messages.length > 0) {
            ...
            if (lastMessage.sender === 'ai' && !isLoading) {
                // Extract plain text for TTS (ignore JSON blocks)
                const { text } = parseLearningPathJSON(lastMessage.text);
                if (text) {
                    speak(text);
                }
            }
        }
    }, [messages, isVoiceActive, isLoading, speak]);
    ...
    const handleSendMessage = async (messageText, files) => {
        ...
        try {
            ...
            const data = await apiChatMessage({ prompt: messageText, images: imagesBase64 });
